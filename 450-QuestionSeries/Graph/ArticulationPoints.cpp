...
//Articulation Point are the vertices on whose removal the graph will break down into two disconnected components.
...
void dfs(int node, int parent, vector<int> &vis, vector<int> &tin, vector<int> &low, int &timer, vector<int> adj[], vector<int> &isArticulation) {
    ...
    //for graph starting with a single vertex and then preceding in 2 directions 
    if(parent == -1 && child > 1) {
        isArticulation[node] = 1; 
    }
}
